...
const addElementEngine = () => {
  ...
  // else if negative value and expenses category
    ...
    const newLi = document.createElement('li')
    // add new text, icon to HTML
    newLi.innerHTML = `<span><i class="fas fa-hand-holding-usd"></i> ${nameInput.value}: ${sumInput.value} <i class="fas fa-times-circle"></i></span>`
...
const fundsEngine = (newSum) => {
  ...
  if(startFunds === 0) {
    alert('Przekroczyłeś swoje przychody!')
    removeAllEngine()
  } else {
